...
int main(int argc, char **argv)
{
    ...
    std::string kernels[] = {"matrix_multiplication_1", "matrix_multiplication_2"};

    for (int i = 0; i < 2; i++) {
        std::cout << "Kernel: " << kernels[i] << std::endl;
        ...
        ocl::Kernel matrix_multiplication_kernel(matrix_multiplication, matrix_multiplication_length, kernels[i]);
...

        {
            ...
            for (int iter = 0; iter < benchmarkingIters; ++iter) {
                ...
                unsigned int global_work_size_x = N;
                unsigned int global_work_size_y = M;
                if (i == 1) {  // костыль костыльный, но так было быстрее закодить
                    work_group_size_x /= 4;
                    global_work_size_x /= 4;
                }

                matrix_multiplication_kernel.exec(
                        gpu::WorkSize(work_group_size_x, work_group_size_y,
        ...
        std::cout << "Average difference: " << diff_avg * 100.0 << "%" << std::endl << std::endl;
        if (diff_avg > 0.01) {
