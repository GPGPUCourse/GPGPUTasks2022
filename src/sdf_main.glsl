...
vec4 sdBody(vec3 p)
{
    float d0 = 0.0, d1 = 0.0, d = 1e10;
    p -= vec3(0.0, 0.20, 0.0);

    // body, two spheres with smoothmin
    d0 = sdSphere((p - vec3(0.0, 0.10, -0.7)), 0.30);
    d1 = sdSphere((p - vec3(0.0, 0.65, -0.7)), 0.10);

    d = smoothmin(d0, d1, 1.0);
    // hands, two capsules, can wave with lazycos
    d0 = sdCapsule(p, vec3(-0.2, 0.4, -0.6), vec3(-0.4, 0.3 - lazycos(iTime * 7.0) / 10.0, -0.6), 0.07);
    d1 = sdCapsule(p, vec3(0.2, 0.4, -0.6), vec3(0.4, 0.3 - 0.1, -0.6), 0.07);
    d = min(d, d0);
    d = min(d, d1);

    // legs, two capsules
    d0 = sdCapsule(p, vec3(0.1, 0.2, -0.65), vec3(0.15, -0.20, -0.65), 0.07);
    d1 = sdCapsule(p, vec3(-0.1, 0.2, -0.65), vec3(-0.15, -0.20, -0.65), 0.07);
    d = min(d, d0);
    d = min(d, d1);
