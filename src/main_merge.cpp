...
int main(int argc, char **argv) {
    ...
    result_gpu.resizeN(n);
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            unsigned int global_work_size = round_up(n, workGroupSize);
            for (int merged_src_len = 1; merged_src_len < n; merged_src_len *= 2) {
              merge.exec(gpu::WorkSize(workGroupSize, global_work_size), as_gpu, result_gpu, n, merged_src_len);
              std::swap(as_gpu, result_gpu);
            }
            t.nextLap();
        }
