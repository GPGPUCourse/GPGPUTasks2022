...
int main(int argc, char **argv) {
    ...
    gpu::gpu_mem_32f as_gpu, tmp_gpu;
    as_gpu.resizeN(n);
    tmp_gpu.resizeN(n);
    {
        ...
        for (int iter = 0; iter < benchmarkingIters; ++iter) {
            ...
            for (int part_size = 1; part_size < n; part_size *= 2) {
                merge.exec(gpu::WorkSize(workGroupSize, global_work_size), as_gpu, tmp_gpu, n, part_size);
