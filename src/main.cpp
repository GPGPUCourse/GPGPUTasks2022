...
int main() {
    ...
    for (unsigned int platformIdx = 0; platformIdx < platformCount; ++platformIdx) {
        ...
        for (unsigned int deviceIdx = 0; deviceIdx < deviceCount; ++deviceIdx) {
            ...
            if (((platformIdx == 0) & (deviceIdx == 0)) | (deviceType == CL_DEVICE_TYPE_GPU)) {
                mainDevice = device;
            }
        }
    }
    size_t mainDeviceNameSize;
    OCL_SAFE_CALL(clGetDeviceInfo(mainDevice, CL_DEVICE_NAME, 0, nullptr, &mainDeviceNameSize));
    std::vector<unsigned char> mainDeviceName(mainDeviceNameSize);
    OCL_SAFE_CALL(clGetDeviceInfo(mainDevice, CL_DEVICE_NAME, mainDeviceNameSize, mainDeviceName.data(), nullptr));
    std::cout << "Device: " << mainDeviceName.data() << std::endl;
    ...
    // см. clBuildProgram
    OCL_SAFE_CALL(clBuildProgram(program, 1, &mainDevice, nullptr, nullptr, nullptr));

    // А также напечатайте лог компиляции (он будет очень полезен, если в кернеле есть синтаксические ошибки - т.е. когда clBuildProgram вернет CL_BUILD_PROGRAM_FAILURE)
    // Обратите внимание, что при компиляции на процессоре через Intel OpenCL драйвер - в логе указывается, какой ширины векторизацию получилось выполнить для кернела
    // см. clGetProgramBuildInfo
    size_t log_size = 0;
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, mainDevice, CL_PROGRAM_BUILD_LOG, 0, nullptr, &log_size));
    std::vector<char> log(log_size, 0);
    OCL_SAFE_CALL(clGetProgramBuildInfo(program, mainDevice, CL_PROGRAM_BUILD_LOG, log_size, log.data(), nullptr));
    if (log_size > 1) {
    ...
    // TODO 10 Выставите все аргументы в кернеле через clSetKernelArg (as_gpu, bs_gpu, cs_gpu и число значений, убедитесь, что тип количества элементов такой же в кернеле)
    {
        ...
        OCL_SAFE_CALL(clSetKernelArg(kernel, i++, sizeof(float), &n));
    }
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        // - Среднее время выполнения кернела равно t.lapAvg() секунд
        std::cout << "VRAM bandwidth: " << (double)(3 * n * sizeof(float)) / t.lapAvg() / (1 << 30) << " GB/s" << std::endl;
    }
    ...
    // Если написать c[index] = a[index] + b[index] + 1 вместо c[index] = a[index] + b[index], то ошибка ловится
