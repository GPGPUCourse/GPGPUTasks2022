...
int main() {
    ...
    for (int platformIndex = 0; platformIndex < platformsCount; ++platformIndex) {
        ...
        std::vector<unsigned char> platfromVendor(platformVendorSize, 0);
        OCL_SAFE_CALL(clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, platformVendorSize, platfromVendor.data(), nullptr));
        std::cout << "    Platform vendor: " << platfromVendor.data() << std::endl;
        ...
        for (int deviceIndex = 0; deviceIndex < devicesCount; ++deviceIndex) {
            ...
            std::cout << "        Device memory: " << deviceMemSize / 10e5 << std::endl;

            cl_bool isLittleEndianDevice;
            OCL_SAFE_CALL(clGetDeviceInfo(device, CL_DEVICE_ENDIAN_LITTLE, sizeof(cl_bool), &isLittleEndianDevice, nullptr));
            std::cout << "        Device is " << (isLittleEndianDevice ? "little endian" : "big endian") << std::endl;
