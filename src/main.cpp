...
cl_device_id choose_device() {
    ...
    throw std::runtime_error("No matching devices found (GPU or CPU)");
}
...
int main() {
    ...
    cl_mem asBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, as.data(), &error);
    reportError(error, __FILE__, __LINE__);

    cl_mem bsBuffer = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, sizeof(float) * n, bs.data(), &error);
    reportError(error, __FILE__, __LINE__);

    cl_mem csBuffer = clCreateBuffer(context, CL_MEM_WRITE_ONLY, sizeof(float) * n, nullptr, &error);
    reportError(error, __FILE__, __LINE__);
    ...
    auto program_source = kernel_sources.c_str();
    cl_program program = clCreateProgramWithSource(context, 1, &program_source, nullptr, &error);
    reportError(error, __FILE__, __LINE__);
    ...
    // 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        timer t;
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(queue, csBuffer, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, nullptr));
            t.nextLap();
        }
