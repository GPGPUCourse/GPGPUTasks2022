...
public:
    ...
    DataManage(T data, F on_release) : on_release(on_release), data(data) {}

    ~DataManage() {
        OCL_SAFE_CALL(on_release(data));
    }
};
...
void printDeviceInfo(cl_device_id device) {
    ...
    std::cout << "\tDevice type: " << device_type_string(deviceType) << std::endl << std::endl;
}
...
int main() {
    ...
    DataManage<cl_command_queue> queue(getCommandQueue(context.data, device), clReleaseCommandQueue);
    ...
    OCL_SAFE_CALL(clEnqueueWriteBuffer(
        queue.data, as_buf.data, CL_TRUE, 0, sizeof(float) * n, as.data(), 0, nullptr, nullptr
    ));

    OCL_SAFE_CALL(clEnqueueWriteBuffer(
        queue.data, bs_buf.data, CL_TRUE, 0, sizeof(float) * n, bs.data(), 0, nullptr, nullptr
    ));
    ...
    DataManage<cl_program> program_aplusb(
        RET_SAFE_CALL(cl_program, clCreateProgramWithSource, context.data, 1, &kernel_sources_c_str, &sources_size),
        clReleaseProgram
    );
    ...
    OCL_SAFE_CALL(clGetProgramBuildInfo(
        program_aplusb.data, device, CL_PROGRAM_BUILD_LOG, 0, nullptr, &log_size
    ));

    std::vector<char> log(log_size, 0);
    OCL_SAFE_CALL(clGetProgramBuildInfo(
        program_aplusb.data, device, CL_PROGRAM_BUILD_LOG, log_size, log.data(), nullptr
    ));
    ...
    //   - Дождаться завершения полунного события - см. в документации подходящий метод среди Event Objects
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            ...
            OCL_SAFE_CALL(clEnqueueNDRangeKernel(
                queue.data, kernel.data, 1, nullptr, &global_work_size, &workGroupSize, 0, nullptr, &event
            ));
    ...
    // 15 Скачайте результаты вычислений из видеопамяти (VRAM) в оперативную память (RAM) - из cs_gpu в cs (и рассчитайте скорость трансфера данных в гигабайтах в секунду)
    {
        ...
        for (unsigned int i = 0; i < 20; ++i) {
            OCL_SAFE_CALL(clEnqueueReadBuffer(
                queue.data, cs_buf.data, CL_TRUE, 0, n * sizeof(float), cs.data(), 0, nullptr, nullptr
            ));
            t.nextLap();
        }
