...
//    float sizeX = 2.0f;
    ...
    context.activate();
    {
        ...
        double relIterationsFraction = 0.0;
        for (int j = 0; j < height; ++j) {
          for (int i = 0; i < width; ++i) {
            relIterationsFraction += gpu_results.ptr()[j * width + i];
          }
        }

        std::cout << "    Real iterations fraction: " << 100.0 * relIterationsFraction / (width * height) << "%" << std::endl;
