                   ...
                   unsigned int iters, bool smoothing)
{
    ...
    for (int j = 0; j < height; ++j) {
        for (int i = 0; i < width; ++i) {
            float x0 = fromX + (i + 0.5f) * scale;
            float y0 = fromY + (j + 0.5f) * scale;
...
void renderInWindow(float centralX, float centralY, unsigned int iterationsLimit, unsigned int aaIters, bool useGPU);
...
int main(int argc, char **argv)
{
    ...
    unsigned int benchmarkingItersGPU = 10;
    unsigned int benchmarkingItersCPU = 5;

    unsigned int width = 2048;
    unsigned int height = 2048;
    unsigned int iterationsLimit = 272;
    unsigned int aaIterations = 5;
...
//    float sizeX = 2.0f;
    ...
    {
        timer t;
        for (int i = 0; i < benchmarkingItersCPU; ++i) {
    ...
    context.activate();
    {
        ...
        uint groupSizeX = 128;
        gpu::WorkSize workSize(groupSizeX, 1, (width + groupSizeX - 1) / groupSizeX * groupSizeX, height);

        timer t;
        for (int i = 0; i < benchmarkingItersGPU; ++i) {
          kernel.exec(workSize, gpu_image,
                      centralX - sizeX / 2.0f, centralY - sizeY / 2.0f, width, height, sizeX / width,
                      iterationsLimit, 1);
          t.nextLap();
        }

        gpu_image.readN(gpu_results.ptr(), width * height);
...
//    bool useGPU = true;
//    renderInWindow(centralX, centralY, iterationsLimit * 3 / 2, aaIterations, useGPU);
...
void renderInWindow(float centralX, float centralY, unsigned int iterationsLimit, unsigned int aaIters, bool useGPU)
{
    ...
    do {
        if (!useGPU) {
            ...
            kernel.exec(gpu::WorkSize(128, 1, width, height),
                        results_vram,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        width, height,
                        sizeX / width,
                        iterationsLimit, aaIters);
