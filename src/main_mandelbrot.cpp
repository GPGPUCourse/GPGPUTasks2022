...
//    float sizeX = 2.0f;
...

    {
        ...
        size_t opportunity1000 = 1000;
        size_t gflops = 1000*1000*opportunity1000;
    ...
    context.activate();
    {
        ...
        timer t;
        for (int i = 0; i < benchmarkingIters; ++i) {
            kernel.exec(gpu::WorkSize(group_size, group_size, width * group_size, height * group_size),
                        results_vram,
                        width, height,
                        centralX - sizeX / 2.0f, centralY - sizeY / 2.0f,
                        sizeX, sizeY,
                        iterationsLimit, (unsigned  int)1);
